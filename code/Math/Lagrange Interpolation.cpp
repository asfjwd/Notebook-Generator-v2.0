/***
 * Description: A polynomial of degree d can be uniquely identified given its values on d + 1 unique points. O(n) to pre-calculate given the first n points (x=0 to n-1) of the polynomial. Then answer each query to interpolate the x'th term in O(n). All values are done modulo mod, which needs to be a prime as we need its inverse modulo. Also includes an additional helper function called find\_degree(terms, mod). Given at least the first d+2 points of a polynomial of degree d, it finds d in roughly O(n log d). Note, n should not exceed mod due to the way modular inverse is used. In such cases, we can use interpolation without modulo in big integers and take the remainder later
 * Time: $O(n)$
***/
